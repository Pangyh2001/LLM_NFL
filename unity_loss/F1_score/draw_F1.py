...
# 加载数据
random_data = load_json('./run-blackbox-tag-F1_Score.json')
recovered_data = load_json('./run-local-tag-F1_Score.json')
...
ax1.set_xscale('log')
ax1.set_ylim(0.81, 0.865)  # 调整范围以更好地显示差异
ax1.set_ylabel('F1 Score', fontsize=10)
ax1.set_title('Effect of Privacy Budget (ε) on F1 Score', 
...
ax2.set_xscale('log')
ax2.set_ylim(0, 0.04)  # 调整范围以更好地显示损失
ax2.set_xlabel('Privacy Budget (ε)', fontsize=10)
ax2.set_ylabel('F1 Score Loss', fontsize=10)
...
# 保存图表
plt.savefig('./F1_score/f1_score_analysis.pdf', format='pdf', bbox_inches='tight', dpi=300)
plt.savefig('./F1_score/f1_score_analysis.eps', format='eps', bbox_inches='tight', dpi=300)
